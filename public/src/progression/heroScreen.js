...
// A world-space HEIGHT offset above the hero's feet, not read off the rig -- deliberately: fitting
// anything to the actual skeleton (a bone, a hand transform) is exactly the "gear fit" work AGENTS.md
// reserves for the asset stream (see tools/blender/fit_gear.py's own header on why that is precise,
// measured work). Tuned by measurement, not guessed: the GP1-C1/C2 closeout review's own harness
// projected this mesh through the Hero screen's actual preview camera (HERO_SCREEN_PREVIEW_DISTANCE
// in main.js) and found the original 1.85m guess landed above the frustum entirely on a portrait
// viewport, and even in-frustum heights below that were depth-occluded by the spawn-clearing's own
// tree landmark and never actually painted a pixel -- both invisible despite mesh.visible === true,
// which is exactly the gap between "the state is right" and "a human can see it" AGENTS.md's
// playtest rule exists to catch. 1.1 keeps the mesh below the slots row and clear of the head.
const PREVIEW_MARKER_HEIGHT_METERS = 1.1;
const PREVIEW_MARKER_RADIUS_METERS = 0.14;
const PREVIEW_MARKER_SPIN_RADIANS_PER_SECOND = 1.1;

/**
 * GP1-C2's visible-hero-proof: a small floating marker, unmistakably NOT real gear (a plain
 * octahedron, no attempt at a blade/shield silhouette), positioned in WORLD space above wherever the
 * hero stands and recoloured to the equipped weapon's own swatch (swatchHexFor, the same source the
 * DOM item card and strip already read). Deliberately NOT attached to hero.root or any bone --
 * main.js positions it every frame from `player.position`, the same plain world coordinate the follow
 * camera already tracks, so this never touches character/gear.js's bone-relative-matrix system or
 * the rig at all (see AGENTS.md's "fitting gear to the hero" section for why that boundary matters).
 * Visible only while the Hero screen is open; `dispose()` is provided for symmetry but main.js keeps
 * one for the session, matching how the hero/wolf/villagers are all mounted once and toggled rather
 * than rebuilt.
 *
 * TEMPORARY BY DESIGN, not just by name: the goal (GP1-C2's own text) is proving the causal contract
 * "tap EQUIP -> hero preview visibly changes", not shipping art. Delete this the moment GP9 attaches
 * a real Wildwood Blade model through an asset-stream-owned path.
 *
 * depthTest is deliberately off (renderOrder pushed after everything else): this is a HUD-style proof
 * marker, not a diegetic object competing for space in the scene, and the spawn clearing's own tree
 * landmark sits close enough to the preview camera to depth-occlude it entirely at a normal height --
 * found by projecting the mesh through the camera and sampling render state directly, not by guessing
 * at a fix from the screenshot alone. Drawing on top is the one change that stays correct regardless
 * of whatever the world backdrop happens to be (a real fix for the backdrop itself is explicitly out
 * of scope for this closeout -- see the private engineering archive's GP1-C1/C2 section).
 */
export function createHeroPreviewMarker(scene, THREE) {
  const geometry = new THREE.OctahedronGeometry(PREVIEW_MARKER_RADIUS_METERS, 0);
  const material = new THREE.MeshStandardMaterial({
    color: NEUTRAL_SWATCH_HEX,
    emissive: NEUTRAL_SWATCH_HEX,
  ...
  mesh.name = 'hero-preview-weapon-marker-TEMPORARY';
  mesh.visible = false;
  ...
  /**
   * @param position    world position to hover above (main.js's own `player.position`)
   * @param itemId      the currently equipped item -- recolours only when it actually changes
   * @param visible     Hero-screen-open, driven by the same flag main.js already computes per frame
   * @param deltaSeconds  for the idle spin; 0 is fine (just skips a frame of rotation)
   */
  function update(position, itemId, visible, deltaSeconds) {
    mesh.visible = visible;
    if (!visible) return;
    mesh.position.set(position.x, position.y + PREVIEW_MARKER_HEIGHT_METERS, position.z);
    spin += deltaSeconds * PREVIEW_MARKER_SPIN_RADIANS_PER_SECOND;
    mesh.rotation.y = spin;
    mesh.rotation.x = spin * 0.6;
    if (itemId !== lastItemId) {
      lastItemId = itemId;
      const hex = swatchHexFor(itemId);
