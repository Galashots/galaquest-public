...
```bash
node tools/meshy/image_to_3d.mjs tmp/<asset>.png tmp/<asset>-body --polycount 7000
...
```bash
node tools/meshy/rig_character.mjs <body-task-id> tmp/<asset>-rig --height 2.2
# after explicit authorization for this spend:
node tools/meshy/rig_character.mjs <body-task-id> tmp/<asset>-rig --height 2.2 --go
```
...
```bash
node tools/meshy/animate_character.mjs <rig-task-id> <action-id> tmp/<asset>-clips --name attack
# after explicit authorization for this spend:
node tools/meshy/animate_character.mjs <rig-task-id> <action-id> tmp/<asset>-clips --name attack --go
```
...
```bash
node tools/foundry/verify_native_clip.mjs --body tmp/<asset>-rig/rigged.glb --clip tmp/<asset>-clips/<clip>.glb
node tools/foundry/pose_anatomy.mjs tmp/<asset>-clips/<clip>.glb
```

After accepted clips are merged into the pristine body, recompress and enforce the shipping budget. Use
the actual consumer's existing asset family rather than inventing a new path; for example:

```bash
python tools/budget/recompress_glb.py tmp/<asset>-merged.glb public/assets/enemies/<asset>.glb --size 1024 --quality 85
node tools/budget/glb_budget.mjs public/assets/enemies/<asset>.glb
```
...
For example, an asset lane may plan around a nominal budget and require a production review before a larger ceiling. The operator must still obtain explicit authorization for the specific paid work before using `--go`.
