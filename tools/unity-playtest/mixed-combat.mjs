...
import { cleanupOwnedBrowser } from './owned-browser-cleanup.mjs';
...
try{
  ...
  try {
    if(chrome?.pid) browserDisposal=await cleanupOwnedBrowser(chrome,profile);
  ...
  const report={clientSha:sha,serverSha,mode,manifest,origin:server?.origin,checks,
    result,failure,cleanup:{passed:clean,profile,browserDisposal,chromePid:chrome?.pid,chromeExitCode:chrome?.exitCode,chromeSignalCode:chrome?.signalCode,errors:cleanupErrors},
