...
// ── players[].weaponId ──────────────────────────────────────────────────────────────────────────
//
// Which sword each player is holding. OPTIONAL on the wire, exactly the additive shape
// wolf.modeSeconds and heroes[].maxHp already use and for the same reason: every pre-existing caller
// and fixture decodes byte-identically, so this is additive validation rather than a version bump.
//
// It exists because character/weaponLoadout.js could not answer "which sword is in a SIBLING'S hand"
// -- its own comment named this field's absence as the reason every remote was forced to the
// shipping sword. A child who earns the Wildwood Blade should be seen holding it.

test('players[].weaponId rides optionally, and is absent rather than null when not sent', () => {
  const withWeapon = snapshotMessage(3,
    [{ id: 'p1', x: 0, z: 0, heading: 0, speed: 0, weaponId: 'wildwood_blade' }],
    ENCOUNTER_FIXTURE, EVENTS_FIXTURE);
  assert.equal(roundTrip(withWeapon).players[0].weaponId, 'wildwood_blade');

  const without = snapshotMessage(3,
    [{ id: 'p1', x: 0, z: 0, heading: 0, speed: 0 }], ENCOUNTER_FIXTURE, EVENTS_FIXTURE);
  assert.equal('weaponId' in roundTrip(without).players[0], false,
    'an absent weapon must stay absent, not decode as a null the client has to special-case');
});

test('a weaponId that is not a string is refused rather than drawn', () => {
  // A number here would reach weaponMeshIdFor, miss every row, and silently fall back to the
  // shipping sword -- a lie about a specific item, which is the one thing the old force-the-shipping
  // -sword behaviour was careful never to be.
  for (const bad of [7, {}, [], true]) {
    assert.throws(() => decode(encode({
      v: PROTOCOL_VERSION,
      type: 'snapshot',
      tick: 1,
      players: [{ id: 'p1', x: 0, z: 0, heading: 0, speed: 0, weaponId: bad }],
      encounter: ENCOUNTER_FIXTURE,
      events: [],
    })), ProtocolError, `weaponId ${JSON.stringify(bad)} must be refused`);
  }
});
