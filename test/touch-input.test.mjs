...
import { clampStick, TOUCH_RUN_DEFLECTION } from '../public/src/input/touch.js';

test('touch stick clamps to a unit circle and preserves screen-up semantics', () => {
  assert.deepEqual(clampStick(0, 0), { x: 0, y: 0 });
  assert.deepEqual(clampStick(56, 0), { x: 1, y: 0 });
  assert.deepEqual(clampStick(0, -56), { x: 0, y: 1 });
...
test('the run threshold is reachable by a thumb that has not hit the rim', () => {
  ...
  const nearRim = clampStick(0, -0.9 * 56);
  assert.ok(
