...
export class Diorama {
  constructor(canvas, content) {
    ...
    // (see resize()) doesn't wash out the whole diorama in haze.
  ...
  syncFarm(farmState, now) {
    ...
    farmState.plots.forEach((plot, i) => {
      ...
      if (!plot.cropId) {
        ...
        this.plotStates[i] = { cropId: null, ready: false };
        return;
      }
      ...
      if (!this.cropMeshes[i] || prev.cropId !== plot.cropId) {
        if (this.cropMeshes[i]) this.plotGroups[i].remove(this.cropMeshes[i]);
        const mesh = gen.buildCrop(THREE, cropDef, progress);
        mesh.traverse((o) => { o.userData.pickType = 'plot'; o.userData.plotIndex = i; });
      ...
      this.plotStates[i] = { cropId: plot.cropId, ready };
    });
  }
  ...
  syncArmor(equippedDefs, forSaleDef) {
    ...
    if (forSaleId !== this.mannequinArmorId) {
      Object.values(this.mannequinSlots).forEach((s) => s.clear());
  ...
  syncEgg(eggState) {
    ...
    for (let i = cracksShown; i < eggState.cracks; i++) {
    ...
    if (eggState.hatched && this.eggVisible) {
      ...
      this.sparkles.spawn(this.eggGroup.position.clone().add(new THREE.Vector3(0, 0.6, 0)), '#ffffff', 28);
  ...
  update() {
    ...
    // Subtle camera sway for a living, breathing diorama.
    this.camera.position.x = this.cameraBase.x + Math.sin(this._sway * 0.25) * 0.35;
